...
from common import format_output, get_cached_or_fetch
...
def _fetch_opencode_stats_uncached() -> dict:
    try:
...
def get_opencode_usage() -> dict:
    return get_cached_or_fetch("opencode", _fetch_opencode_stats_uncached, ttl=120)
...
def main() -> None:
    ...
    parser.add_argument(
        "--format",
        type=str,
        help="Custom format string for waybar text. Available: {icon}, {icon_plain}, {sessions}, {messages}, etc.",
    )
    parser.add_argument(
        "--tooltip-format",
        type=str,
